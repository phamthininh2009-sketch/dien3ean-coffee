export default function ProductCard({ product }) {
  return (
    <div className="group flex flex-col overflow-hidden rounded-2xl border border-ink/10 bg-white transition-shadow duration-300 hover:shadow-xl hover:shadow-ink/10">
      <div className="relative h-64 overflow-hidden bg-cream-soft">
        <img
          src={product.image}
          alt={product.name}
          loading="lazy"
      ...
      <div className="flex flex-1 flex-col p-6">
        <h3 className="font-serif-heading text-xl leading-snug">{product.name}</h3>
        ...
        <button className="mt-5 inline-flex items-center justify-center gap-2 rounded-full border border-ink/20 px-5 py-2.5 text-[12px] font-semibold uppercase tracking-[0.12em] text-ink transition-colors hover:border-rust hover:bg-rust hover:text-cream">
          Khám phá cà phê
        </button>
      </div>
    </div>
